...
func TestCLICreatesGitHubProjectWithQuotedNameAndOutputModes(t *testing.T) {
	...
	var out bytes.Buffer
	if err := RunCLI(c, &out, "", []string{"projects", "github-create", "Quoted, Project", repoURL}, false, false); err != nil {
	...
	out.Reset()
	if err := RunCLI(c, &out, "", []string{"projects", "github-create", "JSON, GitHub Project", repoURL}, false, true); err != nil {
...
func TestCLIGitHubCreationErrorsAreActionableAndCredentialSafe(t *testing.T) {
	...
	githubErr := RunCLI(c, &bytes.Buffer{}, "", []string{"projects", "github-create", "GitHub", "Project", "https://github.com/acme/repo"}, false, false)
	if githubErr == nil {
...
func TestCLIPulseJSONUsesStructuredProjection(t *testing.T) {
	...
	if strings.Contains(out.String(), "schedule-paused") || strings.Contains(out.String(), "Paused schedule") {
		t.Fatalf("pulse JSON included disabled schedule: %s", out.String())
	}
}
