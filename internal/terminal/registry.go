...
type agentPluginActionDefinition struct {
	key         string
	description string
	names       []agentPluginActionName
	examples    []string
}

var agentPluginActions = []agentPluginActionDefinition{
	{
		key:         "list",
		description: "list marketplaces, plugins, and runtime status",
		names: []agentPluginActionName{
			{word: "list", syntax: "agents plugins [list]", summary: true, help: true},
			{word: "state", syntax: "agents plugins state", summary: true, help: true},
			{word: "status", syntax: "agents plugins status", summary: true, help: true},
		},
		examples: []string{`agents plugins`},
	},
	{
		key:         "marketplaces",
		description: "manage plugin marketplaces",
		names: []agentPluginActionName{
			{word: "marketplaces", summary: true},
			{word: "marketplace"},
		},
	},
	{
		key:         "install",
		description: "install a plugin and optionally enable it for an agent",
		names: []agentPluginActionName{
			{word: "install", syntax: "agents plugins install <plugin-id> [agent]", summary: true, help: true},
		},
		examples: []string{`agents plugins install stagehand@official reviewer`},
	},
	{
		key:         "uninstall",
		description: "uninstall a plugin (--force in CLI)",
		names: []agentPluginActionName{
			{word: "uninstall", syntax: "agents plugins uninstall <plugin-id>", summary: true, help: true},
			{word: "remove", syntax: "agents plugins remove <plugin-id>", summary: true, help: true},
		},
		examples: []string{`agents plugins uninstall playwright@official`},
	},
	{
		key:         "enable",
		description: "enable an installed plugin for an agent",
		names: []agentPluginActionName{
			{word: "enable", syntax: "agents plugins enable <agent> <plugin-id>", summary: true, help: true},
		},
		examples: []string{`agents plugins enable reviewer playwright@official`},
	},
	{
		key:         "disable",
		description: "disable an installed plugin for an agent",
		names: []agentPluginActionName{
			{word: "disable", syntax: "agents plugins disable <agent> <plugin-id>", summary: true, help: true},
		},
	},
}

var agentPluginMarketplaceActions = []agentPluginActionDefinition{
	{
		key:         "add",
		description: "add a plugin marketplace",
		names: []agentPluginActionName{
			{word: "add", syntax: "agents plugins marketplaces add <source>", summary: true, help: true},
		},
		examples: []string{`agents plugins marketplaces add github.com/example/plugins`},
	},
	{
		key:         "sync",
		description: "sync a plugin marketplace",
		names: []agentPluginActionName{
			{word: "sync", syntax: "agents plugins marketplaces sync <marketplace>", summary: true, help: true},
			{word: "update", syntax: "agents plugins marketplaces update <marketplace>", summary: true, help: true},
		},
	},
	{
		key:         "remove",
		description: "remove a plugin marketplace (--force in CLI)",
		names: []agentPluginActionName{
			{word: "remove", syntax: "agents plugins marketplaces remove <marketplace>", summary: true, help: true},
			{word: "delete", syntax: "agents plugins marketplaces delete <marketplace>", summary: true, help: true},
		},
	},
	{
		key:         "reset",
		description: "restore default marketplaces (--force in CLI)",
		names: []agentPluginActionName{
			{word: "reset", syntax: "agents plugins marketplaces reset", summary: true, help: true},
			{word: "reset-defaults", syntax: "agents plugins marketplaces reset-defaults", summary: true, help: true},
		},
	},
}

func lookupAgentPluginAction(defs []agentPluginActionDefinition, word string) (agentPluginActionDefinition, bool) {
	word = strings.ToLower(strings.TrimSpace(word))
	for _, def := range defs {
		for _, name := range def.names {
			if strings.EqualFold(name.word, word) {
				return def, true
			}
		}
	}
	return agentPluginActionDefinition{}, false
}

func agentPluginActionSummary(defs []agentPluginActionDefinition) string {
	var names []string
	for _, def := range defs {
		for _, name := range def.names {
			if name.summary {
				names = append(names, name.word)
			}
		}
	}
...
func agentPluginUnknownUsage(path string, defs []agentPluginActionDefinition) string {
	return "usage: " + cmdPrefix + strings.TrimSpace(path+" "+agentPluginActionSummary(defs))
}

func agentPluginCanonicalUsage(defs []agentPluginActionDefinition, key string) string {
	for _, def := range defs {
		if def.key != key {
			continue
		}
		for _, name := range def.names {
			if name.help && name.syntax != "" {
				return "usage: " + cmdPrefix + name.syntax
			}
		}
	}
	return agentPluginUnknownUsage("agents plugins", agentPluginActions)
}

func agentPluginUsageLines() []string {
	var lines []string
	appendLines := func(defs []agentPluginActionDefinition) {
		for _, def := range defs {
			for _, name := range def.names {
				if name.help && name.syntax != "" {
					lines = append(lines, fmt.Sprintf("%-60s %s", name.syntax, def.description))
				}
			}
		}
	}
	appendLines(agentPluginActions)
	appendLines(agentPluginMarketplaceActions)
	return lines
}

func agentPluginExamples() []string {
	var examples []string
	appendExamples := func(defs []agentPluginActionDefinition) {
		for _, def := range defs {
			examples = append(examples, def.examples...)
		}
	}
	appendExamples(agentPluginActions)
	appendExamples(agentPluginMarketplaceActions)
	return examples
}

func agentPluginCompletionValues(defs []agentPluginActionDefinition) []string {
	var values []string
	for _, def := range defs {
		for _, name := range def.names {
			if name.summary {
				values = append(values, name.word)
			}
		}
	}
	return values
...
func agentsPluginsCommand(m Model, c *client.Client, projectID string, args []string) (Model, tea.Cmd) {
	...
	action := strings.ToLower(args[0])
	rest := args[1:]
	def, ok := lookupAgentPluginAction(agentPluginActions, action)
	if !ok {
		return m, errCmd(agentPluginUnknownUsage("agents plugins", agentPluginActions))
	}
	switch def.key {
	case "list":
		if len(rest) != 0 {
			return m, errCmd(agentPluginCanonicalUsage(agentPluginActions, "list"))
		}
	...
	case "enable", "disable":
		return agentsPluginToggleCommand(m, c, projectID, def.key == "enable", rest)
	default:
		return m, errCmd(agentPluginUnknownUsage("agents plugins", agentPluginActions))
	}
}
...
func agentsPluginMarketplacesCommand(m Model, c *client.Client, args []string) (Model, tea.Cmd) {
	...
	if !ok {
		return m, errCmd(agentPluginUnknownUsage("agents plugins marketplaces", agentPluginMarketplaceActions))
	}
	switch def.key {
	case "add":
		if len(rest) != 1 || strings.TrimSpace(rest[0]) == "" {
			return m, errCmd(agentPluginCanonicalUsage(agentPluginMarketplaceActions, "add"))
		}
	...
	case "sync":
		if len(rest) != 1 || strings.TrimSpace(rest[0]) == "" {
			return m, errCmd(agentPluginCanonicalUsage(agentPluginMarketplaceActions, "sync"))
		}
	...
	case "remove":
		if len(rest) != 1 || strings.TrimSpace(rest[0]) == "" {
			return m, errCmd(agentPluginCanonicalUsage(agentPluginMarketplaceActions, "remove"))
		}
		name := strings.TrimSpace(rest[0])
	...
	case "reset":
		if len(rest) != 0 {
			return m, errCmd(agentPluginCanonicalUsage(agentPluginMarketplaceActions, "reset"))
		}
...
func agentsPluginInstallCommand(m Model, c *client.Client, projectID string, args []string) (Model, tea.Cmd) {
	if len(args) < 1 || strings.TrimSpace(args[0]) == "" {
		return m, errCmd(agentPluginCanonicalUsage(agentPluginActions, "install"))
	}
...
func agentsPluginUninstallCommand(m Model, c *client.Client, args []string) (Model, tea.Cmd) {
	if len(args) != 1 || strings.TrimSpace(args[0]) == "" {
		return m, errCmd(agentPluginCanonicalUsage(agentPluginActions, "uninstall"))
	}
	pluginID := strings.TrimSpace(args[0])
...
func agentsCommand() command {
	...
	usage = append(usage, agentPluginUsageLines()...)
	examples := []string{
	...
	examples = append(examples, agentPluginExamples()...)
	examples = append(examples, `agents metrics`, `agents votes step-exec-123`)
	return command{
		name:         "agents",
		...
		completions: []commandCompletion{
			...
			{after: []string{"plugins"}, values: agentPluginCompletionValues(agentPluginActions)},
			{after: []string{"plugins", "marketplaces"}, values: agentPluginCompletionValues(agentPluginMarketplaceActions)},
			{after: []string{"plugins", "marketplace"}, values: agentPluginCompletionValues(agentPluginMarketplaceActions)},
		},
...
func validateChannelAPIEndpoint(value string) error {
	if value == "" {
		return nil
	}
	u, err := url.Parse(value)
	if err != nil || (u.Scheme != "http" && u.Scheme != "https") || u.Host == "" {
		return errors.New("--api-endpoint must be an absolute HTTP(S) URL")
	}
	return nil
}

func validateChannelEmailAddress(value string) error {
	if value == "" {
		return nil
	}
	address, err := mail.ParseAddress(value)
	if err != nil || address.Address != value || !strings.Contains(address.Address, "@") {
		return errors.New("--address must be a valid email address")
	}
	return nil
}

func normalizeGitHubPrivateKey(value string) (string, error) {
	value = strings.TrimSpace(strings.ReplaceAll(strings.ReplaceAll(value, `\\n`, "\n"), `\n`, "\n"))
	if block, rest := pem.Decode([]byte(value)); block != nil && strings.TrimSpace(string(rest)) == "" {
		return string(pem.EncodeToMemory(block)), nil
	}
	for _, blockType := range []string{"RSA PRIVATE KEY", "PRIVATE KEY"} {
		begin := "-----BEGIN " + blockType + "-----"
		end := "-----END " + blockType + "-----"
		beginAt, endAt := strings.Index(value, begin), strings.Index(value, end)
		if beginAt < 0 || endAt <= beginAt+len(begin) {
			continue
		}
		body := strings.Join(strings.Fields(value[beginAt+len(begin):endAt]), "")
		candidate := begin + "\n" + body + "\n" + end + "\n"
		if block, rest := pem.Decode([]byte(candidate)); block != nil && strings.TrimSpace(string(rest)) == "" {
			return string(pem.EncodeToMemory(block)), nil
		}
	}
	return "", errors.New("--private-key must be valid PEM")
}

func validateChannelConfiguration(action, channelType string, values map[string]string) error {
	if err := validateChannelAPIEndpoint(values["github_api_endpoint"]); err != nil {
		return err
	}
	if err := validateChannelEmailAddress(values["email_address"]); err != nil {
		return err
	}
...
func channelWizardTransitionRequiresExplicit(wizard *channelWizardState, field string) bool {
	...
	case "github":
		...
		if mode == wizard.original.Get("github_auth_mode") {
			return false
		}
		return (mode == "pat" && field == "github_pat") ||
			(mode == "app" && (field == "github_app_id" || field == "github_app_slug" || field == "github_app_private_key"))
	case "slack":
		return wizard.form.Get("slack_bot_token_mode") == "manual" && wizard.original.Get("slack_bot_token_mode") != "manual" && field == "slack_bot_token"
	case "email":
		return wizard.form.Get("email_provider") == "custom" && wizard.original.Get("email_provider") != "custom" && (field == "email_imap_host" || field == "email_smtp_host")
	}
	return false
}

func channelWizardFieldRequired(wizard *channelWizardState, step channelWizardStep) bool {
	if channelWizardTransitionRequiresExplicit(wizard, step.field) {
		return true
	}
	if step.required {
		return true
...
func (m Model) beginChannelWizard(action string, channel client.Channel) (Model, tea.Cmd) {
	original := channel.EditableSettings()
	form := make(url.Values, len(original))
	for key, values := range original {
		form[key] = append([]string(nil), values...)
	}
	if action == "add" {
		original = make(url.Values)
	...
	m.channelWizard = &channelWizardState{action: action, channel: channel, original: original, form: form, supplied: make(map[string]bool), steps: channelWizardSteps(action, channel.Type), restorePrompt: m.input.Prompt, restoreHint: m.input.Placeholder}
	m.menu = nil
	m.input.SetValue("")
...
func validateChannelWizardValue(step channelWizardStep, value string) error {
	if step.field == "github_api_endpoint" {
		return validateChannelAPIEndpoint(value)
	}
	if step.field == "email_address" {
		return validateChannelEmailAddress(value)
	}
...
func channelWizardValidationValues(wizard *channelWizardState) map[string]string {
	if wizard.action == "add" {
		return channelWizardValueMap(wizard.form)
	}
	values := make(map[string]string, len(wizard.supplied))
	for field := range wizard.supplied {
		if (field == "github_auth_mode" || field == "slack_bot_token_mode" || field == "email_provider") && wizard.form.Get(field) == wizard.original.Get(field) {
			continue
		}
		values[field] = wizard.form.Get(field)
	}
	return values
}
...
func (m Model) handleChannelWizardKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "enter":
		...
		value := strings.TrimSpace(m.input.Value())
		m.input.SetValue("")
		if value != "" && step.field == "github_app_private_key" {
			normalized, err := normalizeGitHubPrivateKey(value)
			if err != nil {
				m.append(entry{role: "error", text: err.Error()})
				m.setChannelWizardPrompt()
				return m, nil
		...
		requiresExplicit := channelWizardTransitionRequiresExplicit(wizard, step.field)
		if value == "" && channelWizardFieldRequired(wizard, step) && (requiresExplicit || wizard.form.Get(step.field) == "") {
		...
		if value != "" {
			...
			wizard.supplied[step.field] = true
		}
		wizard.index++
		if wizard.index < len(wizard.steps) {
			m.setChannelWizardPrompt()
		...
		if err := validateChannelConfiguration(action, channel.Type, channelWizardValidationValues(wizard)); err != nil {
			wizard.index = 0
			m.append(entry{role: "error", text: err.Error()})
			m.setChannelWizardPrompt()
			return m, nil
...
func parseChannelMutationArgs(action string, args []string) (client.Channel, map[string]string, error) {
	...
	for i := 2; i < len(args); i += 2 {
		...
		if backendField == "github_app_private_key" {
			value, err = normalizeGitHubPrivateKey(value)
			if err != nil {
				return client.Channel{}, nil, err
			}
		}
...
func channelAccessProvider(provider string) (string, error) {
	...
	return "", errors.New("channel access provider must be telegram, slack, discord, x, email, or github")
}
...
func normalizeChannelAccessIdentity(provider, value string) (string, error) {
	...
	case "x":
		if !numericAccessUserID.MatchString(value) {
			return "", errors.New("X access requires a numeric user ID")
		}
...
func normalizeXChannelAccessUsername(value string) (string, error) {
	value = strings.TrimSpace(value)
	if !xAccessUsername.MatchString(value) {
		return "", errors.New("X access username must be a valid username")
	}
...
func validateChannelAccessArgs(args []string) error {
	...
	case "add":
		...
		if provider == "github" {
			...
			if len(args) == 5 && strings.TrimSpace(args[4]) == "" {
				return errors.New("GitHub display name cannot be empty")
			}
		}
		if provider == "x" {
			if len(args) > 5 {
			...
			if len(args) == 5 {
				if _, err := normalizeXChannelAccessUsername(args[4]); err != nil {
					return err
				}
			}
		}
	case "remove":
		if provider == "github" {
			if len(args) > 4 || (cliMode && len(args) == 3) {
				return errors.New(channelAccessUsage())
			}
...
func channelAccessTitle(provider string) string {
...
func channelAccessUserLabel(user client.ChannelAuthorizedUser) string {
	...
	if user.Provider == "x" && displayName != "" && !strings.HasPrefix(displayName, "@") {
		displayName = "@" + displayName
	}
	if user.Provider == "github" {
		login := strings.TrimPrefix(identity, "@")
		if login != "" {
			login = "@" + login
		}
		switch {
		case displayName == "" || strings.EqualFold(strings.TrimPrefix(displayName, "@"), strings.TrimPrefix(identity, "@")):
			return login
		case login == "":
			return displayName
		default:
...
// xChannelAccessOutputUser is deliberately narrower than the legacy shared
// access view: X output contains only the canonical row identity, selected
// project, numeric X user ID, and optional username.
type xChannelAccessOutputUser struct {
	ID        string `json:"id"`
	ProjectID string `json:"project_id"`
...
func channelAccessXOutputUser(user client.ChannelAuthorizedUser) xChannelAccessOutputUser {
	return xChannelAccessOutputUser{
		ID:        sanitizeAutomationDetailText(user.ID),
		ProjectID: sanitizeAutomationDetailText(user.ProjectID),
		XUserID:   sanitizeAutomationDetailText(user.Identity),
		Username:  strings.TrimPrefix(sanitizeAutomationDetailText(user.DisplayName), "@"),
	}
}

func renderChannelAccessJSON(provider string, users []client.ChannelAuthorizedUser) (string, error) {
	if provider == "x" {
		out := make([]xChannelAccessOutputUser, 0, len(users))
		for _, user := range users {
			out = append(out, channelAccessXOutputUser(user))
		}
		return marshalJSON(out)
	}
...
type xChannelAccessActionJSON struct {
	Action   string                   `json:"action"`
	Provider string                   `json:"provider"`
	User     xChannelAccessOutputUser `json:"user"`
}

func renderChannelAccessUsers(provider string, users []client.ChannelAuthorizedUser) string {
	if len(users) == 0 {
		return "No authorized " + channelAccessTitle(provider) + " access identities."
	}
	var b strings.Builder
	fmt.Fprintf(&b, "Authorized %s access\n", channelAccessTitle(provider))
	for _, user := range users {
		if provider == "x" {
			username := strings.TrimPrefix(sanitizeAutomationDetailText(user.DisplayName), "@")
			fmt.Fprintf(&b, "  record ID: %s  project ID: %s  X user ID: %s", sanitizeAutomationDetailText(user.ID), sanitizeAutomationDetailText(user.ProjectID), sanitizeAutomationDetailText(user.Identity))
			if username != "" {
...
func channelAccessMutationResult(ctx context.Context, c *client.Client, projectID, provider, action string, user client.ChannelAuthorizedUser) (string, error) {
	...
	case "add":
		if provider == "github" {
			err = c.AddGitHubAuthorizedActor(ctx, projectID, user.Identity, user.DisplayName)
		} else {
			err = c.AddChannelAuthorizedUser(ctx, provider, projectID, user.Identity, user.DisplayName)
		}
	case "remove":
		if provider == "github" {
			err = c.RemoveGitHubAuthorizedActor(ctx, projectID, user.ID)
		} else {
	...
	status := fmt.Sprintf("%s %s access for %q", map[string]string{"add": "authorized", "remove": "removed"}[action], channelAccessTitle(provider), channelAccessUserLabel(user))
	users, reloadErr := c.ListChannelAuthorizedUsers(ctx, provider, projectID)
	if (provider == "x" || provider == "github") && action == "add" && reloadErr == nil {
		matches := channelAccessUserMatches(users, func(existing client.ChannelAuthorizedUser) bool {
			return existing.ProjectID == projectID && existing.MatchesIdentity(user.Identity)
		})
		if len(matches) == 1 {
			user = matches[0]
		}
	...
	if jsonMode {
		if provider == "x" {
			return marshalJSON(xChannelAccessActionJSON{
				Action: action, Provider: provider,
				User: channelAccessXOutputUser(user),
			})
		}
...
func confirmChannelAccessRemoval(m Model, projectID, provider string, user client.ChannelAuthorizedUser) (Model, tea.Cmd) {
	...
	return confirmOr(m,
		fmt.Sprintf("Remove %s access for %q? Type 'yes' to confirm or Esc to cancel.", channelAccessTitle(provider), label),
		fmt.Sprintf("use --force to confirm removal of %s access for %q", channelAccessTitle(provider), label),
		cmd)
}

func channelAccessSelector(m Model, provider string) (Model, tea.Cmd) {
	projectID, c := m.selectedID, m.client
	return selectorOr(m, channelAccessUsage(), selectorFor("Authorized "+channelAccessTitle(provider)+" access", "channels access "+provider+" remove", "no authorized access identities configured", false, func(ctx context.Context) ([]selectorItem, error) {
		users, err := c.ListChannelAuthorizedUsers(ctx, provider, projectID)
		if err != nil {
			return nil, err
		}
...
func runChannelAccess(m Model, args []string) (Model, tea.Cmd) {
	...
	case "list":
		return m, run("Channel access", cmdTimeout, func(ctx context.Context) (string, error) {
			...
			if jsonMode {
				return renderChannelAccessJSON(provider, users)
			}
	...
	case "add":
		...
		if provider == "x" && displayName != "" {
			displayName, _ = normalizeXChannelAccessUsername(displayName)
		}
...
func channelsCommand() command {
	...
	return command{
		...
		usage: []string{
			...
			"GitHub PAT mode requires --pat; app mode requires --app-id, --app-slug, and --private-key. Pasted multiline PEM is supported.",
			"GitHub API endpoints must be absolute HTTP(S) URLs; Email addresses must be valid mailbox addresses.",
			"Changing GitHub/Slack auth mode or Email to custom requires the target mode's credentials or hosts.",
		...
		run: func(m Model, args []string) (Model, tea.Cmd) {
			...
			if action == "disconnect" {
				return confirmOr(m, fmt.Sprintf("Disconnect channel %q? Type 'yes' to confirm or Esc to cancel.", ch.Name), fmt.Sprintf("use --force to confirm disconnect of channel %q", ch.Name), runAction)
			}
			return m, runAction
		},
	}
	...
	return command{
		...
		run: func(m Model, args []string) (Model, tea.Cmd) {
			if cliMode {
				...
				return m, errCmd("events on must run through the foreground CLI stream; use the openvibely-terminal events command")
			}
			if len(args) > 1 {
