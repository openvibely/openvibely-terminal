// Package terminal implements the OpenVibely terminal UI and one-shot CLI.
//
...
func tagMessage(msg tea.Msg, sessionGeneration, projectGeneration uint64) tea.Msg {
	...
	case agentDeleteTargetMsg:
		...
		typed.projectGeneration = projectGeneration
		return typed
	case scheduleDeleteTargetMsg:
		typed.sessionGeneration = sessionGeneration
		typed.projectGeneration = projectGeneration
		return typed
	case taskDeleteTargetMsg:
		typed.sessionGeneration = sessionGeneration
		typed.projectGeneration = projectGeneration
		return typed
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case resultMsg:
		...
		if msg.refreshTaskID != "" && msg.refreshProjectID != "" &&
			msg.refreshTaskID == m.threadID && msg.refreshProjectID == m.selectedID {
			return m, m.refreshTaskThread(msg.refreshTaskID, msg.refreshProjectID, "")
		}
		return m, nil
	...
	case agentDeleteTargetMsg:
		...
		return confirmAgentDeletion(m, msg.projectID, msg.agent)
	...
	case scheduleDeleteTargetMsg:
		...
		return confirmScheduleDeletion(m, msg.projectID, msg.schedule)
	...
	case taskDeleteTargetMsg:
		...
		return confirmTaskDeletion(m, projectID, msg.task)
...
func OfflineRecoveryMessage(baseURL string, err error) string {
	...
	if isRemoteServerURL(baseURL) {
		b.WriteString("  - Check or correct the configured remote server URL first, then run /status or openvibely-terminal -server <url> status.\n")
		b.WriteString("  - Use -server <url> or OPENVIBELY_SERVER_URL to point at the running remote backend.\n")
		b.WriteString("  - Run /setup in the TUI, or openvibely-terminal setup in a shell, for read-only connection guidance.\n")
	} else {
		b.WriteString("  - Run /setup in the TUI, or openvibely-terminal setup in a shell, for read-only installation and startup steps.\n")
