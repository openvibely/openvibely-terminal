...
func TestLoadAutomationDefinitionUsesScopedBuilderContract(t *testing.T) {
	...
	s := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write([]byte(`<div id="automation-builder" data-automation-id="au-1" data-project-id="p2"><textarea name="automation_yaml">schema_version: 1
name: Keep &amp; edit
</textarea></div>`))
	}))
	defer s.Close()
...
func TestUpdateAutomationDefinitionPreviewsBeforeSaving(t *testing.T) {
	...
	s := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if len(forms) == 1 {
			_, _ = w.Write([]byte(`<div id="automation-builder" data-automation-id="au-1" data-project-id="p1"><textarea name="automation_yaml">ok</textarea></div>`))
			return
		}
...
func TestUpdateAutomationDefinitionValidationFailureDoesNotSave(t *testing.T) {
	...
	s := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		requests++
		_, _ = w.Write([]byte(`<div id="automation-builder" data-automation-id="au-1" data-project-id="p1"><div data-automation-validation-summary><ul><li>node trigger is required</li></ul></div><textarea name="automation_yaml">invalid</textarea></div>`))
	}))
	defer s.Close()
	c, _ := New(s.URL)
...
func TestUpdateAutomationDefinitionParseFailureDoesNotSave(t *testing.T) {
	...
	s := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		requests++
		_, _ = w.Write([]byte(`<div id="automation-builder" data-automation-id="au-1" data-project-id="p1"><div role="alert">YAML did not parse: invalid mapping</div><textarea name="automation_yaml">invalid</textarea></div>`))
	}))
	defer s.Close()
	c, _ := New(s.URL)
	err := c.UpdateAutomationDefinition(context.Background(), "p1", "au-1", "invalid")
