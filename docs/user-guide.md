...
openvibely-terminal projects github-create "My GitHub Project" https://github.com/acme/demo
```
...
`projects github-create <name> <url>`; use `|` before the URL when the project
name contains spaces. The backend performs the clone and GitHub
...
openvibely-terminal --json projects github-create "GitHub Project" https://github.com/acme/demo # JSON GitHub project record
...
$ openvibely-terminal help projects
  ...
  projects github-create <name> <url>           create and select a GitHub-backed project
    use | before the URL when the project name contains spaces
...
| Pulse | `--json pulse` builds a scoped structured upcoming-work projection from `GET /api/tasks/reference-catalog?project_id=...` and the structured schedule cards at `GET /schedule?project_id=...` |
...
while `--json pulse` combines the compact task catalog with structured schedule
card attributes to avoid chat/model-dependent output. Personality custom detail
