...
## Reconnect Timer Test Seams

When a reconnect command intentionally waits in production, separate construction of the reconnect message from timer scheduling or inject a narrow scheduler seam. Tests should use an immediate scheduler/message path so named recovery tests do not sleep through the live delay, while retaining every generation, submission, project, execution, stale-scope, cancellation, and UTF-8 byte-offset assertion. Keep the default scheduler's exact production delay unchanged and add a focused regression that invokes the injected scheduler and asserts both that delay and all scheduled message fields; this keeps the scheduling callback covered instead of creating a coverage hole.

Measure focused timing with a platform-appropriate wall-clock method rather than assuming GNU `time` flags on macOS. Treat unrelated high-CPU work as environment evidence when it contaminates repeated timing samples. If a workflow harness extracts an unsubstituted GitHub Actions expression such as `${{ inputs.uncached }}` into the shell, report the harness limitation and, if useful, validate only the harness logic with a temporary non-repository substitution; do not alter production workflow files to accommodate the broken harness.
