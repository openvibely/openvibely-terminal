...
Use this note when debugging or extending Tab/Enter behavior for slash commands in `internal/terminal/`.
...
- `internal/terminal/model.go` — Bubble Tea key handling and the Tab/Enter paths.
- `internal/terminal/command.go` — command lookup, quote-aware tokenization, completion helpers, structural boundary predicates, and menu candidates.
- `internal/terminal/registry.go` — registry metadata and dispatch cases.
- `internal/terminal/selector.go` and `internal/terminal/messages.go` — searchable selector state and dispatch/prefill messages.
- `internal/terminal/model_test.go`, `internal/terminal/command_test.go`, `internal/terminal/selector_test.go`, and `internal/terminal/dispatch_test.go` — key-path, completion, selector, and CLI/dispatch regressions.
