...
Use this reference when auditing or changing project selection, project loading, or login/session recovery in `internal/tui/`.
...
Relevant areas are project-selection and project-load/login handlers in `internal/tui/model.go`, their message types in `internal/tui/messages.go`, and end-to-end model tests using the existing `httptest`/SSE test helpers. Run the focused regression and the standard `go test ./... -count=1` suite after implementation.
